...
select
    distinct i_funcionarios 
from
    bethadba.funcionarios
where
    i_entidades = 1
    and
    i_funcionarios not in (
    select
        i_funcionarios
    from
...
join bethadba.hist_funcionarios b on (a.i_pessoas = b.i_pessoas)
where b.i_funcionarios = (select max(x.i_funcionarios) from bethadba.hist_funcionarios x where x.i_pessoas = b.i_pessoas)
and b.dt_alteracoes = (select max(s.dt_alteracoes) from bethadba.hist_funcionarios s where s.i_funcionarios = b.i_funcionarios) 
and a.i_funcionarios in (
    select
    z.i_funcionarios
from
    bethadba.funcionarios z
where
    i_entidades = 1
    and
    i_funcionarios not in (
