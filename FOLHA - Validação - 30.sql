...
-- CORREÇÃO
-- Atualiza os históricos salariais com data de alteração maior que a data de rescisão, ajustando a data de alteração para um minuto após a última alteração ou para o primeiro dia do mês da data de rescisão se não houver alterações anteriores
...
for a1 as a2 cursor for
	select hs.i_funcionarios,
           ...
           xSQL = 'update bethadba.hist_salariais
					  set dt_alteracoes = '''||dt_alteracoes_novo||'''
					where i_funcionarios = '||hs.i_funcionarios||'
					  and i_entidades = '||hs.i_entidades||'
					  and dt_alteracoes = '''||hs.dt_alteracoes||''';'
      from bethadba.hist_salariais hs
     inner join bethadba.rescisoes r
        on (hs.i_funcionarios = r.i_funcionarios
       and hs.i_entidades = r.i_entidades)
