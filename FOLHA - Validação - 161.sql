...
select distinct         
    ...
    hf.i_funcionarios,
        ...
        hf.i_organogramas,              
        hc.i_cargos,
        num_sintetico = (select no3.tot_digitos  from bethadba.niveis_organ no3 where no3.i_config_organ = hf.i_config_organ and no3.i_niveis_organ = organogramas.nivel  - 1),
    sintetico = left(hf.i_organogramas, num_sintetico) + repeat('0',(select no3.num_digitos from bethadba.niveis_organ no3 where no3.i_config_organ = hf.i_config_organ and no3.i_niveis_organ = organogramas.nivel))
    from
        bethadba.funcionarios,
        bethadba.hist_cargos hc,
        bethadba.hist_funcionarios hf, (select  entidade=f.i_entidades,
    ...
    where  
        funcionarios.i_entidades in (1,2,3,4) and
    ...
    hf.i_config_organ = organogramas.i_config_organ and hf.i_organogramas = organogramas.i_organogramas and
        ...
        exists(select 1 from bethadba.cargos_organogramas co
        where co.i_entidades = hf.i_entidades and co.i_cargos = hc.i_cargos)        
        and not exists(select 1 from bethadba.cargos_organogramas co
        where co.i_entidades = hf.i_entidades and co.i_cargos = hc.i_cargos
 ...
 */
     ...
     delete from BETHADBA.cargos_organogramas
